...
\subsection{其他结构特性}
\begin{frame}[fragile]\ft{\subsecname}
\red{现在的C允许把一个结构赋值给另一个结构。} \vspace{0.1in}
...
\begin{frame}[fragile]\ft{\subsecname}
\red{也可以把一个结构初始化为另一个同样类型的结构。} \vspace{0.1in}
...
\begin{frame}[fragile]\ft{\subsecname}
\red{现在的C中，结构不仅可作为参数传递给函数，也可以作为函数返回值返回。} \vspace{0.1in}
...
\begin{frame}[fragile]\ft{\subsecname}
  接下来看看如何使用\red{结构参数和返回值}来完成这个任务。
\end{frame}
...
\item 对于\verb|struct pnames|变量\verb|name2|，字符串存储在编译器存储字符串常量的任何地方。该结构存放的只是两个地址，总共栈16个字节。\red{（注：所有类型的指针变量在32位系统上都是4字节， 64位系统上都是8字节。）}\\[0.1in]
\item[]
...
\begin{frame}[fragile]\ft{\subsecname}
  \begin{itemize}
  \item \red{如果需要一个结构来存储字符串，请使用字符数组成员。} \\[0.1in]
